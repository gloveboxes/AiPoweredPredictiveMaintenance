...
bool co2_initialize(int fd)
{
    ...
    if (error)
    {
        Log_Debug("Error executing scd4x_get_serial_number(): %i\n", error);
    }
    else
    {
        Log_Debug("serial: 0x%04x%04x%04x\n", serial_0, serial_1, serial_2);
    }
    ...
    if (error)
    {
        Log_Debug("Error executing scd4x_start_periodic_measurement(): %i\n", error);
    }
